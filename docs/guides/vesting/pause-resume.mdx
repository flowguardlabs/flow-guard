...
<Warning>
The current public backend does not expose `/api/streams/:id/pause` and `/api/streams/:id/resume` endpoints yet. This guide documents contract behavior and planned API support.
</Warning>
...
Pause/resume is available in `VestingCovenant` and related services, but the HTTP routes are not wired in the current API build.
