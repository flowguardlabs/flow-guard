...
<Warning>
`transfer()` is implemented in `VestingCovenant`, but the current backend release does not expose `/api/streams/:id/transfer` yet.
</Warning>
...
Recipient transfer is contract-supported and planned for API wiring in a future backend release.
